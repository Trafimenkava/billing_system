<?php
	...
	$international_calls = $_POST["internationalcalls"];
	...
	foreach($tariff_plans as $tariff_plan) {
		...
		$result .= "<p>" . replaceNullByZero($tariff_plan['internet_traffic_mb']) . " МБ Интернета, ";
		$result .= replaceNullByZero($tariff_plan['phone_traffic_within_network_min']) . " мин внутри сети и ";
		$result .= replaceNullByZero($tariff_plan['phone_traffic_all_networks_min']) . " мин во все сети, ";
